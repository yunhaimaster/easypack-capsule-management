...
1. **Prompt 內部不要有空行**：每個 Prompt 的所有內容連續書寫，中間只用逗號、句號分隔
2. **⚠️ Prompt 之間必須留兩個換行符（一個空行）⚠️**：這是強制要求！每個 Prompt 結束後，按兩次 Enter 鍵，留一個空行，然後才開始下一個 Prompt 標題
3. **一個段落**：每個 Prompt 寫成一個流暢的段落（600-800 字）
4. 格式：[主色...] [背景...] [排除...]，攝影類型...，構圖要求...，產品描述...，光線...，道具...，氛圍...，技術規格...
5. 不要寫「第一部分」「第二部分」等標題

**⚠️ 錯誤示範（不要這樣）**：
\`\`\`
**實拍瓶身 Prompt:** [內容...]技術規格高解析度。
**情境 Prompt:** [內容...]構圖使用三分法則。  ← ❌ 沒有空行！會導致提取錯誤！
**香港製造 Prompt:** [內容...]香港旅遊傳照。
\`\`\`
...
**檢查方式**：輸出後，數一數有沒有 5 個 Prompt（實拍、情境、平鋪、香港、海報）和 4 個空行（Prompt 之間）。
