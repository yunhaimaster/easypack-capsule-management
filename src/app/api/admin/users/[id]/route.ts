...
// Update user role or nickname (Admin and Manager)
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updateData: { role?: Role; nickname?: string | null } = {}
    ...
    if (Object.keys(updateData).length === 0) {
      return NextResponse.json({ success: false, error: '沒有要更新的數據' }, { status: 400 })
    }
    ...
    return NextResponse.json({ success: false, error: '更新用戶失敗' }, { status: 500 })
  }
}
