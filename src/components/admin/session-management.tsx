...
export function SessionManagement({ selectedUserId, onClearFilter }: SessionManagementProps) {
  ...
  const loadData = async () => {
    try {
      ...
      if (data.success) {
        ...
        // 獲取用戶電話號碼（用於顯示篩選標題）
        if (selectedUserId && data.sessions.length > 0) {
          const phone = data.sessions[0]?.user?.phoneE164
