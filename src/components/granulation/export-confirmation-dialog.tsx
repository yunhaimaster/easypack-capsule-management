      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-gray-800">訂單資訊</h3>
          </div>
          
          <div className="bg-blue-50/50 border border-blue-200/60 rounded-lg p-4 space-y-2">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs text-gray-600 mb-1">客戶名稱</p>
            ...
            <div>
              <p className="text-xs text-gray-600 mb-1">產品名稱</p>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-800">配方原料</h3>
            <Badge 
          ...
          {hasIngredients ? (
            <div className="bg-white border border-gray-200 rounded-lg overflow-hidden max-h-64 overflow-y-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                  <tr>
                    <th className="px-4 py-2 text-left text-xs font-medium text-gray-600">原料名稱</th>
                    <th className="px-4 py-2 text-right text-xs font-medium text-gray-600">單粒含量</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {order.ingredients.map((ingredient, index) => (
                    <tr key={ingredient.id || index} className="hover:bg-gray-50/50">
                      <td className="px-4 py-2.5 text-gray-800">{ingredient.materialName}</td>
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-red-800">無法匯出配方</p>
                <p className="text-xs text-red-700 mt-1">該訂單目前沒有配方原料資料。</p>
              </div>
            </div>
          )}
        </div>
